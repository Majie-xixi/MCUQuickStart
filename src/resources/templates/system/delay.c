...
static uint16_t  g_fac_us = 72*5;      /* us延时倍乘数 */
...
void delay_init(uint16_t sysclk)
{
	...
	SysTick->CTRL |= (1 << 2);  // ← 改成1，使用HCLK不分频
    g_fac_us = sysclk;          // 108MHz，每us就是108个tick
...
{
    uint32_t temp;
    SysTick->CTRL = (1 << 2);           // ?? 先设置时钟源HCLK，同时关闭计数
    SysTick->LOAD = nus * g_fac_us;     // 时间加载
    SysTick->VAL  = 0x00;               // 清空计数器
    SysTick->CTRL = (1 << 2) | (1 << 0); // 时钟源HCLK + 开始倒数
